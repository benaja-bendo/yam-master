...
import type { Cell, GameContext, GameState } from "@yamaster/logic";
...
export const GameBoard: React.FC<Props> = ({ context, send }) => {
  ...
  const { dice, keptDice, rollsLeft } = context;

  const handleRoll = () => send({ type: "ROLL" });
  const handleKeep = (idx: number) =>
    send({ type: "KEEP", diceIndexes: [idx] });
  ...
  if (context.players) {
    Object.entries(context.players).forEach(
      ([playerId, playerData]: [string, Record<string, any>]) => {
  ...
  const availableCombinations = state.context.dice
    ? evaluateCombinations(state.context.dice)
    : [];
